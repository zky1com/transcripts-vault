---
title: "The $540 Million A Year Pizza Business"
people_mentioned: [Sam Parr, Val]
channel: My First Million
video_id: yaI4UFXkptI
url: "https://www.youtube.com/watch?v=yaI4UFXkptI"
publish_date: ""
duration: ""
word_count: 199
content_type: clip
delivery_mode: short
broad_category: entrepreneurship
subcategories: [distribution, franchising, food business]
series_name: My First Million
episode_id: yaI4UFXkptI
primary_person: Sam Parr
host_names: [Sam Parr]
interviewer_names: []
interviewee_profiles: []
speaker_profiles: [Sam Parr]
organizations_mentioned: []
locations_mentioned: []
tools_mentioned: []
companies_mentioned: [Hunt Brothers Pizza]
topics: [distribution, food business, embedded retail]
tags: []
pipeline_stage: video_only
claims_status: done
speech_status: pending
world_status: pending
evidence_status: pending
claims_count: 1
speech_evidence_count: 0
world_evidence_count: 0
evidence_count: 0
batch_id: ""
last_processed_at: "2026-03-19"
---

## Summary

Sam Parr uses Hunt Brothers Pizza as an example of how a business can become huge by changing the distribution model instead of reinventing the product. The company turned pizza into a tiny embedded store-inside-a-store format that piggybacks on existing convenience-store traffic and labor.

## Key Insights

- The innovation is distribution and footprint, not gourmet differentiation.
- Embedding the business inside another retailer reduces the need for standalone real estate.
- The format works because convenience stores already have traffic and operators in place.
- The model reframes pizza as an add-on revenue stream for an existing merchant.

## Claims

- [[Sam Parr - Hunt Brothers Pizza grew by embedding tiny pizza businesses inside convenience stores instead of opening full restaurants]]

## Full Transcript

<details>
<summary>Click to expand full transcript (199 words)</summary>

Our buddy Val tweeted this out. He goes, "The biggest pizza chain in the country is one I've never heard of, Hunt Brothers Pizza. These guys went in and basically found a new way to do the pizza business. They've spread to 9,000 stores. I'm pretty sure they're doing over 500 million a year in revenue. What these guys do is they created a pizza restaurant that is only like 50 or 100 square feet. The way it works is they put them into convenience stores, gas stations. The owner of the gas station now has a little pizza shop inside. So he can pop in a pizza. It takes five minutes. It comes out broiling hot and they can sell them by the slice or as they call them by the hunk. A hunk of pizza is 1/4 of a pizza and they sell it for like $2.89 or you can get two hunks for $5.49. Basically you buy the equipment for 10 grand but then after that there's no contract, there's no licensing fee, there's no royalty fee. So it's very different than a traditional franchise. All you do is you have to buy the ingredients from

</details>

