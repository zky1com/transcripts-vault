...
people_mentioned: [Roma, Maya, Alex]
channel: OpenAI
...
delivery_mode: demo
broad_category: ai
subcategories: [coding agents, code review, developer tools]
series_name: OpenAI
episode_id: HwbSWVg5Ln4
primary_person: Maya
host_names: [Roma]
interviewer_names: [Roma]
interviewee_profiles: [Maya]
speaker_profiles: [Roma, Maya]
organizations_mentioned: [OpenAI]
locations_mentioned: []
tools_mentioned: [Codex, Codex CLI, GitHub, VS Code, Python]
companies_mentioned: [OpenAI, GitHub, VS Code]
topics: [ai coding, verification, code review, developer workflows]
...
Roma and Maya show how Codex handles code review as part of a broader coding workflow rather than as a narrow static-analysis tool. The demo emphasizes that stronger coding agents create a new verification problem, so OpenAI is training review-specific models that can inspect full repositories, run tools, test hypotheses, and help teams catch important bugs before they reach production.

## Key Insights

- The pitch for Codex review is that verification needs to scale alongside code generation, not trail behind it.
- The demo positions code review as a repository-aware reasoning task rather than simple diff inspection.
- Precision matters as much as recall because noisy review bots quickly become ignored by engineering teams.
- Steerability through repo-level instructions is treated as a first-class requirement for useful automated review.
- The product is designed to fit both cloud PR review and local pre-PR review workflows.

## Claims

- [[Maya - Stronger coding agents make verification the new bottleneck, so review models must improve alongside generation models]]
- [[Maya - High-quality AI code review requires repository context and active investigation, not just reading the diff]]
- [[Maya - Useful review bots need very high precision because developers will ignore tools that create too many false alarms]]
- [[Maya - Repo-level instructions make automated code review more practical because teams can steer what the model should care about]]
- [[Roma - Automated review is valuable because it lets teams explore more possible failure cases than busy human reviewers usually have time to run]]
