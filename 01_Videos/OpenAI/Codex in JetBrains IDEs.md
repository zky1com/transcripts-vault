...
delivery_mode: demo
broad_category: ai
subcategories: [developer tools, ides, coding agents]
series_name: OpenAI
episode_id: 1XkVsE9-ZK4
primary_person: Gleb
host_names: []
interviewer_names: []
interviewee_profiles: [Gleb]
speaker_profiles: [Gleb]
organizations_mentioned: [OpenAI, JetBrains, Kotlin Conf]
locations_mentioned: []
tools_mentioned: [Codex, Codex CLI, JetBrains AI, IntelliJ, Gradle]
companies_mentioned: [OpenAI, JetBrains]
topics: [ides, coding agents, developer workflow, verification]
tags: [ai, codex, jetbrains]
...
This demo introduces Codex inside JetBrains IDEs as a way to bring the same coding agent used in other Codex surfaces into a workflow many developers already trust. The core message is that Codex should work wherever developers prefer to code, while still preserving the same underlying model quality, verification behavior, and parallel collaboration pattern that make agentic coding practical.

## Key Insights

- The integration is framed as location portability for Codex rather than as a separate product.
- The demo emphasizes large-project debugging and localization work rather than toy edits.
- Agent access modes are presented as an important control surface for different risk tolerances.
- The agent is meant to coexist with the developer's normal IDE workflow rather than interrupt it.
- Verification through builds and checks is treated as part of the expected loop.

## Claims

- [[Gleb - Codex should work anywhere developers like to code rather than forcing them into one interface]]
- [[Gleb - The JetBrains integration matters because it preserves the same Codex quality as other Codex surfaces]]
- [[Gleb - Agentic coding works best when the model can research the codebase before changing anything]]
- [[Gleb - Developers need different access modes because trust in agents depends on how much autonomy they are willing to grant]]
- [[Gleb - Verification is essential because writing code is not enough if the agent cannot show that the result actually works]]
